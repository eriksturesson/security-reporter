...
/**
 * FEATURE 12: Generate beautiful HTML report
...
<head>
    ...
    <title>Security Report - ${new Date(report.timestamp).toLocaleDateString()}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="container">
        <header>
            ...
            <div class="meta">
                ...
                <div class="meta-item">
                    <span>⏱️</span>
                    <span>${report.executionTime}ms execution time</span>
                </div>
                <div class="meta-item">
                    <span>📊</span>
                    <span>${report.projectType} project</span>
                </div>
            </div>
        </header>
        ...
        <footer>
            ...
            <p>Based on <a href="https://cheatsheetseries.owasp.org/cheatsheets/NPM_Security_Cheat_Sheet.html" target="_blank">OWASP NPM Security Best Practices</a></p>
        </footer>
    </div>
</body>
</html>`;
...
/**
 * Helper: Render individual check card
 */
const renderCheckCard = (check: CheckResult, type: string): string => {
  const detailsHtml = check.details
    ? `
    <div class="check-details">
      <pre>${typeof check.details === "string" ? check.details : JSON.stringify(check.details, null, 2)}</pre>
    </div>
  `
    : "";
  ...
  return `
    <div class="check-card ${type}">
      <div class="check-header">
        <div class="check-name">${check.name}</div>
        <span class="badge ${check.severity}">${check.severity}</span>
      </div>
