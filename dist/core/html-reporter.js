...
exports.saveHtmlReport = exports.reportToHtml = void 0;
...
const reportToHtml = (report) => {
    ...
    const overallColor = report.overallStatus === "pass" ? "#10b981" : report.overallStatus === "warn" ? "#f59e0b" : "#ef4444";
 ...
 */
const renderCheckCard = (check, type) => {
  ...
  `
        : "";
    const suggestionsHtml = check.suggestions && check.suggestions.length > 0
        ? `
 ...
 */
const saveHtmlReport = (report, outputPath) => {
    const html = (0, exports.reportToHtml)(report);
    const defaultPath = path.join(process.cwd(), `security-report-${new Date(report.timestamp).toISOString().split("T")[0]}.html`);
